...
## [2026-07-12] create | Temporal 워크플로우 오케스트레이션 엔진 완전 정리
- created: [[temporal-workflow-orchestration-comprehensive]] — AWS Graviton 개념 설명에 이어진 대화에서, "Temporal 도입"이라는 문장 하나에서 출발해 Q&A로 깊어진 Temporal/SAGA 심층 정리를 논리 순서(배경→핵심→심화→종합 예제)로 재구성. (1) **Temporal 정의·구성요소**: Durable Execution, Workflow/Activity/Worker/Event History 역할. (2) **SAGA 패턴과의 관계**: Choreography vs Orchestration, Temporal = Process Manager 패턴 구현체, Self-hosted vs Temporal Cloud "직접 배포"의 의미. (3) **보상 트랜잭션**: 두 방식 공통 개념이나 트리거 주체가 다름, 보상 트랜잭션 자체의 실패로 인한 불일치 리스크와 대응 전략(멱등성, 정합성 검증 배치). (4) **Workflow 상태 관리**: order_id=Workflow ID로 트랜잭션 단위 상태 관리, 오케스트레이션 상태(Temporal 소유) vs 도메인 상태(각 MS 소유) 구분, Persistence 2계층(Default Store/Visibility Store) 물리 구조, WorkflowIdConflictPolicy(Fail/UseExisting/TerminateExisting)와 Signal-With-Start. (5) **Activity와 외부 통신**: Task Queue ≠ Message Broker 구분, 동기(gRPC)/비동기(Broker+Async Completion) 두 패턴, task_token 기반 콜백 코드, 멱등키 설계 오류 정정(Activity ID는 멱등키 부적합), Broker 자체 재전송과의 이중 retry 문제와 레이어 분리 해법, Kafka와 내부 Task Queue의 역할 분담 및 "Kafka가 앞단에 있으면 Temporal scale-out은 문제 없다"는 흔한 오해 정정. (6) **Heartbeat**: 4가지 목적(Worker 생존/진행 체크포인트/취소 전달/비동기 완료 대기) 정리, "raise_complete_async 호출 시 메모리에 살아있다"는 오해 정정, Activity 단위 vs Workflow Task Timeout 구분, heartbeat_timeout 장기 설정의 숨은 비용과 백그라운드 감시 잡 병행 전략. (7) **결합도 트레이드오프**: Temporal/Space/Design-Time Coupling 3분류로 "비동기가 결합을 없애는 게 아니라 종류를 바꾼다"는 통찰, gRPC 동기 호출의 결합을 Temporal Retry Policy가 흡수하는 메커니즘. (8) **종합 예제**: 배달 서비스(결제→조리·배차 병렬→알림) SAGA Orchestrator 전체 Python 코드(shared/activities/workflow/worker/starter/resolve_stuck_order)와 정상/보상실패 시나리오 트레이스, 코드-개념 매핑표. Temporal 공식 문서·블로그, xgrid, Kai Waehner 등 20여 개 출처 교차검증. develop/architecture.

## [2026-07-08] update | PRD의 역할·문서 간 분업·시각자료 범위 보충
- updated: [[lean-startup-product-dev-workflow-deep-research]] — 6️⃣ PRD 작성 절에 (1) PRD는 "결정을 기록하는 문서"가 아니라 "결정을 만들어가는 도구"이며 얼라인이 PR/FAQ(1차, 개략 Go/Kill)와 PRD+Tech Spec 자체(2차, 최종 승인) 두 번 일어난다는 정정 (2) 그럼에도 승인된 PRD는 "완성되면 이런 제품"이라는 팀 공통 acceptance 기준이 된다는 확인 (3) PRD/Design Spec/Tech Spec 3문서 역할분담(무엇을·왜 / 어떻게 보이는가 / 어떻게 구현하는가)과 PRD엔 저해상도 와이어프레임·유저플로우까지만 포함되고 고해상도 디자인은 Design Spec에 링크된다는 점, 전형적 PRD 섹션 구성 예시 추가. 📊 검증 메타데이터 절에 검증 전후 변경 7건 중 "내용/구조 자체 변경"(3건) vs "확신도 라벨만 하향"(3건) vs "부분적"(1건) 구분표 추가. product-management.

## [2026-07-08] create | 린 스타트업 "아이디어 → PRD → 개발" 표준 워크플로우 Deep Research (검증 완료)
- created: [[lean-startup-product-dev-workflow-deep-research]] — 린 스타트업 맥락에서 소프트웨어 제품을 빠르게 만드는 표준 워크플로우(아이디어 발굴→기획/발견→우선순위화→기술검토→기획확정→PRD→개발→출시)를 deep-research로 조사 후, rl-verify로 2라운드(1차 4관점 병렬 + 2차 블라인드 재검증 2관점) 수렴 검증. **확정 오류 4건 수정**: (1) 최초 버전의 파이프라인 다이어그램/비교표(확정→PRD 순)와 본문 절 번호(PRD→...→확정) 간 순서 자기모순 — 통합 순서로 재정렬 (2) 종합 비교표에 "아이디어 발굴" 행 누락 + "기획/발견" 1단계가 2행으로 인위 분할된 구조 결함 수정 (3) "팀 규모별 실전 가이드" 표에 [Synthesized] 태그 누락 및 무출처 문장("1~5인 PRD 생략 가능") 발견 → 태그·출처 명시 (4) Stage-Gate "미국기업 88% 채택" 통계가 벤더(stage-gate.com) 자사 재인용 구조이며 실제 조사기관별 60~88% 편차 존재 → 확신도 [Confirmed]→[Likely] 하향. 그 외 RICE/ICE 전환규칙, PRD-TechSpec 병행 서술, YC MVP 데드라인 구체 수치 등 3건도 단일 출처 근거로 확인되어 하향 권고. BML/PR·FAQ/RICE공식/Shape Up/OST/PoC·Prototype·MVP 3분법 등 핵심 사실은 원문 대조로 정확함 확인. product-management (신설 카테고리).

## [2026-07-07] create | "필수(Blocker)/핵심(Major)/참고(Minor)" 평가기준 표현과 우선순위 프레임워크 조사
- created: [[evaluation-criteria-priority-frameworks]] — Dify 도입 검토 Notion 문서에서 본 "필수(Blocker)/핵심(Major)/참고(Minor)" 3단계 평가기준 표현이 일반적인 표준 용어인지, 그리고 이런 가중치 기반 평가를 돕는 도구가 있는지 deep-research로 조사. (1) **Blocker/Major/Minor 자체는 Jira의 기본 Priority 값**(버그 트래커 severity 도메인 어휘를 의사결정 기준 가중치 목적으로 차용) (2) **정식 대응 프레임워크**: MoSCoW(Must/Should/Could/Won't — Dai Clegg 1994 Oracle→DSDM 표준화)가 가장 가까운 개념, RFC 2119(MUST/SHOULD/MAY — IETF 기술표준 문서 관행)도 유사 대응 (3) **실제 도구**: Weighted Decision Matrix(기준별 숫자 가중치×점수 합산), 벤더 평가 스코어카드(Smartsheet 등 완제품 템플릿)가 build-vs-buy 의사결정에 실무적으로 쓰이는 형태 (4) 권장: 라벨을 MoSCoW로 바꾸거나 각주 병기 + 채점표를 숫자 가중치로 확장. 6개 공식/1차 출처 교차검증(Atlassian, Agile Business Consortium, IETF RFC 2119, Lucid, Smartsheet). research.
