...
> - CGNAT의 과제: ISP가 **공인 IP 수에 비해 가입자가 많아** (예: 공인 IP 1개, 가입자 1000명), 여러 가입자가 같은 공인 IP를 공유.
> - 여러 구현 방식 존재:
>   - **① 포트 블록 할당**: 각 가입자에게 포트 범위를 사전 할당 (예: 가입자A = 1000~1064, 가입자B = 1065~1129, ...). 한 가입자의 고갈이 다른 가입자에게 영향 없음. 개별 고갈: ~65개 포트 범위 한계로 매우 빠름.
>   - **② 동적 할당 + 가입자별 제한**: 포트를 동적으로 할당하되, 각 가입자의 동시 활성 연결 수를 제한 (예: 최대 100개 연결). 포트 반환 시 재사용 ([RFC 6888](https://datatracker.ietf.org/doc/html/rfc6888) 가이드라인 참고). 
>   - **③ 다중 공인 IP**: 여러 공인 IP를 분산 사용 (예: 10개 IP × 1000명). 각 IP당 64K 포트 독립.
> - 결과: CGNAT 가입자는 일반 NAT의 직원보다 훨씬 제한된 포트를 사용하며, 포트 부족을 자주 마주칩니다.
