...
> 🔧 **F5 (포트 고갈 정밀화)**: NAT PAT 고갈은 **(공인IP, 목적지IP:포트) 조합당 ~64K 동시 연결**을 초과할 때 발생합니다 — 이는 포트 번호(1~65535)가 유한하다는 물리적 한계입니다.
> 
> **핵심 메커니즘**: 
> - 한 공인 IP에서 **같은 목적지(예: 203.0.113.1:443)로 가는 모든 연결**은 **출발지 포트를 1~65535 범위에서 선택**해야 합니다.
> - 따라서 그 목적지로 64K개 이상 동시 연결을 시도하면 포트 번호가 고갈되어 **새 연결이 실패**합니다.
> - **다른 목적지(예: 203.0.113.2:443)로 가는 연결은 독립적인 포트 풀을 사용**하므로, 첫 번째 목적지가 고갈되어도 두 번째는 정상입니다.
> 
> **CGNAT 예시**: 가입자 1000명이 모두 **같은 CDN 엣지(1.2.3.4:443)에 접속** → 1000개 동시 연결만 해도 64K 근처에서 더 이상 새 연결 불가. 반대로 **1000명이 다양한 API 엔드포인트(각기 다른 목적지 IP:포트)로 분산** → 목적지별로 독립적인 64K 풀을 가지므로 수백만 동시 연결 가능([RFC 6888](https://datatracker.ietf.org/doc/html/rfc6888#section-3)).
