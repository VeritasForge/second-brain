...
> 🔧 **F5 (포트 고갈 정밀화)**: "포트 고갈"은 사실 두 가지 다른 레벨의 문제를 섞어 부르는 용어입니다.
> 
> **① 포트 숫자 한계 (1~65535 범위)**: 특정 5-tuple이 점유하는 출발지 포트는 OS가 할당합니다 (OS의 ephemeral 포트 풀 범위는 보통 수천~40K 정도). 이건 **단일 NAT 공인 IP의 한계가 아니라 OS의 포트 범위 설정**이고, 같은 공인 IP도 **다른 목적지(IP:포트)로 새 연결**을 개시하면 **새로운 5-tuple이 생성**되어 새로운 OS 포트를 할당받습니다.
> 
> **② 상태테이블 용량 (실제 고갈)**: NAT 장비는 **각 연결의 5-tuple 상태를 메모리에 저장**합니다. 고갈은 "IP당 64K"가 아니라 **NAT 장비의 상태테이블 용량 한계**(예: 100만 엔트리)에 닿을 때 발생합니다. 
> 
> **결과**: CGNAT 가입자 1000명이 **다양한 서버**로 분산 접속하면 1000×서버수 개 5-tuple을 추적하지만, 다 다른 목적지이므로 각각 독립적으로 OS 포트 할당을 받고, NAT 상태테이블도 충분히 분산됩니다. 반대로 **같은 CDN 엣지(IP:포트)로 집중**되면, 1000명이 같은 목적지로 가므로 상태테이블의 그 엔트리 부분에 1000개 5-tuple이 몰려 상태테이블 용량을 빠르게 소진합니다([RFC 6888 CGNAT 개요](https://datatracker.ietf.org/doc/html/rfc6888#section-3)).
