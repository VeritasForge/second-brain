...
> 🔧 **F5 (포트 고갈 정밀화)**: 포트 고갈은 **출발지 공인 IP가 고정되고 목적지(IP:포트)도 같을 때** 발생합니다. 동일 5-tuple(프로토콜, 출발지IP, 출발지포트, 목적지IP, 목적지포트)는 유일하므로, **다른 목적지로 접속하면 포트를 재사용**할 수 있고, 따라서 단일 공인 IP도 충분한 목적지 수다양성이 있으면 64K 포트 한계를 사실상 무시합니다(CGNAT가 성립하는 이유). 
> 
> 고갈은 "IP당 64K 일괄 상한"이 아니라 **같은 목적지(예: 한 CDN 엣지IP:포트)에 집중된 동시 접속**일 때만 발생합니다 — 예를 들어 CGNAT 가입자 1000명이 같은 API 엔드포인트에 동시 접속하면, 그 엔드포인트 튜플당 ~64K개 포트로 1000명을 다 감당할 수 없어 일부 연결이 실패합니다. 반대로 다양한 서버·서비스로 분산된 트래픽은 포트 고갈을 거의 마주치지 않습니다.
