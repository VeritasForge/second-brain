...
> - 여러 구현 방식 존재:
>   - **① 포트 블록 할당** (RFC 6888 § 5): 각 가입자에게 포트 범위를 사전 할당 (예: 가입자A = 1000~1064, 가입자B = 1065~1129, ...). 한 가입자의 고갈이 다른 가입자에게 영향 없음. 개별 고갈: ~65개 포트 범위 한계로 매우 빠름.
>   - **② 동적 할당 + 가입자별 제한**: 포트를 동적으로 할당하되, 각 가입자의 동시 활성 연결 수를 제한 (예: 최대 100개 연결). 포트 반환 시 재사용. 
