...
> 🔧 **F5 (포트 고갈 정밀화)**: NAT PAT 포트 고갈은 **(공인IP, 목적지IP:포트) 조합당 ~64K 동시 연결** 제한입니다. 포트 번호(1~65535)가 유한하기 때문입니다.
> 
> **핵심 메커니즘**: 한 공인 IP에서 같은 목적지(예: 8.8.8.8:443)로 가는 **모든 내부 호스트의 연결**이 출발지 포트 1~65535을 공유합니다. 따라서:
> - 그 목적지로 64K개 이상 동시 연결 시도 → 포트 번호 고갈 → 새 연결 실패
> - **다른 목적지(예: 1.1.1.1:443)로 가는 연결은 독립적인 포트 풀 사용** → 고갈 회피
>
> **일반 NAT 예시** (회사 공인 IP 1개, 직원 100명): 
> - 100명이 같은 API(1.2.3.4:443)에 동시 접속 → 100 << 64K (문제 없음)
> - 극단: 70K명이 같은 API에 집중 → 64K 포트 고갈 → 이후 연결 실패 ([RFC 6888](https://datatracker.ietf.org/doc/html/rfc6888#section-3))
>
> **CGNAT 구조** (RFC 6888 포트 블록 할당):
> - 포트 고갈 메커니즘은 일반 NAT와 동일: **(공인IP, 목적지IP:포트) 당 ~64K** 제한.
> - CGNAT의 특이점: ISP가 공인 IP를 여러 가입자가 공유하므로, 각 가입자에게 **포트 범위(블록)를 사전 할당**합니다 ([RFC 6888 § 5](https://datatracker.ietf.org/doc/html/rfc6888#section-5)).
> - 예: 공인 IP 1개(65K 포트)를 가입자 1000명에게 할당 → 각 가입자 ~65개 포트 블록 (예: 가입자A는 1000~1064, 가입자B는 1065~1129, ...).
> - **각 가입자는 할당받은 블록 내에서만 포트 사용** — 다른 가입자의 포트를 쓸 수 없으므로, 한 가입자의 고갈이 다른 가입자에게 영향 없음.
> - 개별 가입자의 포트 고갈: **(공인IP, 할당 블록, 목적지IP:포트) 당 ~65개** 에서 매우 빠르게 발생 (같은 목적지로 67개 연결 시도하면 고갈).
> - 따라서 CGNAT 가입자는 포트 부족을 훨씬 자주 마주칩니다 (일반 NAT의 직원 100명과 달리, 포트 제약이 ~65개로 극도로 제한).
