...
>
> **CGNAT 예시** (ISP 공인 IP 1개, 가입자 1000명 공유):
> - 1000명이 **모두 같은 CDN(8.8.8.8:443)에 접속** → 각자 다른 가입자ID로 구분 → 각자 독립 포트 풀 할당 → (공인IP, 가입자A, 8.8.8.8:443)는 64K, (공인IP, 가입자B, 8.8.8.8:443)도 64K, ... → 합쳐서 1000×64K 포트 가능
> - 반대로 **가입자 1명이 같은 CDN으로만 70K 동시 연결** → 그 가입자의 (공인IP, 해당 가입자, 8.8.8.8:443) 포트 고갈 → 그 가입자만 새 연결 실패
