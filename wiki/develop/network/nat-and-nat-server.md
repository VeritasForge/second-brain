...
> 🔧 **F5 (포트 고갈 정밀화)**: NAT 장비는 **각 고유한 5-tuple(프로토콜, 출발지IP, 출발지포트, 목적지IP, 목적지포트) 조합당 상태 엔트리를 하나씩 유지**합니다. ephemeral 포트는 OS 단계에서 할당되므로, 각 새로운 연결 시도는 **새로운 포트가 선택되고, 따라서 새로운 5-tuple**을 만듭니다. 
> 
> 고갈은 "단일 공인 IP당 64K 일괄 상한"이 아니라 **NAT 장비가 동시에 추적해야 할 5-tuple의 총 개수**에 의존합니다. 같은 목적지(예: 한 API 엔드포인트)에 집중된 대량 접속 시 그 목적지와의 5-tuple만 늘어나므로, 충분히 다양한 목적지로 분산되면 단일 공인 IP도 64K 포트 한계를 피할 수 있습니다 — 예: CGNAT 가입자 1000명이 **다양한 외부 서버**로 분산 접속하면 포트 고갈 없음. 반대로 **같은 CDN 엣지IP:포트로 집중**되면, 그 엔드포인트 방향의 unique 5-tuple이 ~64K 근처에서 고갈됩니다.
