...
>
> **CGNAT 구조** (RFC 6888 포트 블록 할당, ISP 공인 IP 1개를 가입자 1000명이 공유):
> - ISP는 **각 가입자에게 포트 블록(예: 512개 또는 1024개)을 사전 할당**합니다 ([RFC 6888](https://datatracker.ietf.org/doc/html/rfc6888#section-5)).
> - 예: 가입자A는 1024~1535, 가입자B는 1536~2047, ... (겹치지 않음)
> - **각 가입자는 자신의 블록 내에서만 포트를 동적으로 할당**받아 사용합니다 (블록 내 다양한 목적지로 시간대별 재사용 가능).
> - 따라서 개별 가입자의 포트 고갈은 **(공인IP, 자신의 할당 블록, 목적지IP:포트) 당 512~1024** 에서 발생합니다.
> - 예: 가입자A가 자신의 512개 블록(1024~1535)을 모두 사용 중인 상태에서 같은 목적지로 새 연결 시도 → 그 가입자의 포트 고갈 (다른 가입자는 자신의 블록만 사용하므로 무관) ([RFC 6888 § 5](https://datatracker.ietf.org/doc/html/rfc6888#section-5))
