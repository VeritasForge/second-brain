...
>
> **CGNAT 구조** (ISP 공인 IP 1개를 가입자 1000명이 동적으로 공유):
> - 포트는 **연결 생성 시 동적으로 할당**되고 **연결 종료 시 반환됩니다** (고정 범위가 아님).
> - 전체 (공인IP, 목적지IP:포트) 포트 풀은 여전히 **~64K 제한**이지만, 1000명이 동시에 사용합니다.
> - 따라서 **모든 가입자의 동시 활성 연결이 64K를 초과하면** 누군가 새 연결에 실패합니다 — 어느 가입자인지는 할당 순서에 따라 결정됩니다.
> - 예: 가입자 1~999는 이미 각각 60개 연결 중 (59,940개 포트 사용) → 가입자 1000번이 같은 목적지로 100개 연결 시도 → 가용 포트 60개 < 100개 필요 → 가입자 1000번의 일부 연결 실패 (RFC 6888의 "per-subscriber" 제한과 관계없는 물리적 고갈)
