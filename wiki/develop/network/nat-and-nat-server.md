...
>
> **CGNAT 현실** (ISP, 공인 IP 제약):
> - 포트 고갈은 **동일하게 (공인IP, 목적지IP:포트) 당 ~64K** 제한입니다.
> - 하지만 ISP가 가입자 1000명을 공인 IP 1개로 수용하려면 (고정 할당 불가):
>   - **① 여러 공인 IP 분산**: 공인 IP 10개를 가입자 1000명이 나눔 → 각 IP당 ~64K × 10 = 640K 포트 용량
>   - **② 시간대별 동적 공유**: 같은 공인 IP도 가입자별 동시 활성 연결 수를 제한해 포트 회전 → 연결 종료 시 포트 반환 (RFC 6888의 "per-subscriber" 동시 연결 한계는 이를 위함)
>   - 개별 고갈은 여전히 일반 NAT와 동일한 메커니즘이지만, **가입자별 동시 연결을 사전 제한**해 고갈 상황을 드물게 만듭니다.
