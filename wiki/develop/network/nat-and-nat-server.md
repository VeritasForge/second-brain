...
>
> **CGNAT 구조** (ISP 공인 IP 1개를 가입자 1000명이 공유):
> - ISP는 **공인 IP 1개의 1~65535 포트를 가입자들에게 할당** (예: 각 가입자에게 65K/1000 ≈ 65개 또는 블록 단위 할당)
> - 각 가입자는 **할당받은 포트 범위만 사용 가능** (예: 가입자A는 1024~2047, 가입자B는 2048~3071, ...)
> - 따라서 개별 가입자의 고갈은 **(공인IP, 자신의 할당 포트 범위, 목적지IP:포트) 당 수백~수천** 동시 연결로 훨씬 빨리 발생합니다
> - 예: 가입자A에게 1024개 포트 할당 + 같은 목적지(8.8.8.8:443)로 집중 → 1024개 포트 고갈 → 그 가입자는 새 연결 불가 (다른 가입자는 무관)
