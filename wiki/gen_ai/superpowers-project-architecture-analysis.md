...
#### verification-before-completion 심화

> 💡 비유: 시험 끝나고 "다 맞았어!"라고 말하기 전에, 반드시 답안지를 다시 한번 확인하고 나서야 말할 수 있게 하는 규칙. "아마 맞을 거야"는 절대 허용 안 됨.

**The Gate Function (5단계 검증 게이트)**:

```
"다 했다!"라고 말하려는 순간
   ↓
[1] IDENTIFY — 이 주장을 증명할 명령어가 뭐지?
   ↓
[2] RUN — 해당 명령어를 지금 바로 실행
   ↓
[3] READ — 출력 전체를 읽고, exit code 확인, 실패 수 카운트
   ↓
[4] VERIFY — 출력이 주장을 확인하는가?
   │  ├─ NO → 실제 상태를 증거와 함께 보고
   │  └─ YES → 증거와 함께 주장
   ↓
[5] ONLY THEN — 비로소 "완료"라고 말할 수 있음
```

**Common Failures (대표적 검증 시나리오)**:

| 주장             | 필요한 증거                       | 불충분한 것                          |
| ---------------- | --------------------------------- | ------------------------------------ |
| "테스트 통과"    | 테스트 명령 출력: 0 failures      | 이전 실행 결과, "아마 통과할 거야"   |
| "빌드 성공"      | 빌드 명령: exit 0                 | 린터 통과 (린터 ≠ 컴파일러)          |
| "버그 수정됨"    | 원래 증상 재현 테스트 통과        | 코드 변경했으니 고쳐졌겠지           |
| "요구사항 충족"  | 항목별 체크리스트 검증            | 테스트 통과 (테스트 ≠ 요구사항)      |

**Red Flags (즉시 멈춰야 하는 신호)**:
- "should", "probably", "seems to" 같은 추측성 표현
- 검증 전에 "Great!", "Perfect!", "Done!" 같은 만족 표현
- "이번 한 번만", "자신 있어", "에이전트가 성공했다고 함" 같은 합리화

**탄생 배경**: **24건의 실패 기억**에서 탄생. 파트너가 "I don't believe you"라고 한 사건, 미정의 함수가 배포된 사건 등이 계기. 원문: "Honesty is a core value. If you lie, you'll be replaced."
